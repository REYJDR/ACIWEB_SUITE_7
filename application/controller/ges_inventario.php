...
public function getLocationByItem(){
    ...
    $STATUS_LOC = $this->model->lote_loc_by_itemID($this->ProductID);
            ...
            echo '<tr><td><input class="form-control col-lg-2"    value="'.$LOTE.'" readonly/></td>
                <td><input class="form-control col-lg-2"  value="'.$fab.'" readonly/></td>
                <td><input class="form-control col-lg-2"  value="'.$venc.'" readonly/></td>
                <td><input class="numb form-control col-lg-2"  value="'.number_format(($STOCK_QTY - $qtypend),0, '.', ',').'" readonly/></td>
                <td><input '.$color.' class="numb form-control col-lg-2"  value="'.number_format($qtypend,0, '.', ',').'"  readonly/></td>
...
public function get_almacen_selectlist(){
   ...
   $query_almacen= 'SELECT STOCKS.id, STOCKS.name  
                        ...
                        where STOCKS.onoff="1" AND ( STOCKS.id = "1" OR STOCKS.ID_compania = "'.$this->model->id_compania.'" ) group by STOCKS.id, STOCKS.name ORDER BY STOCKS.name ';
...
public function getListItems(){
    ...
    $columns =  array( 
                ...
                '`stockQTY` as `'.$Tblcol4.'`',
                ' CAST(`QtyOnHand` AS DECIMAL(18,4)) as `'.$Tblcol10.'`',
  ...
  $idStock = $_GET['stock'];
   ...
   if($idStock != 1 ){
        ...
        //actualiza ubicación a default de todos los items asignados 
            $this->model->delete('STOCK_ITEMS_LOCATION',  ' where  stock="'.$idStock.'" and ID_compania = "'.$this->model->id_compania.'"'); 
            $this->CheckError();
        ...
        echo 'El almacen principal no puede ser eliminado';
...
public function removeLoc(){
    ...
    if($idLoc != 1 ){
        ...
        //elimina ubicación a default de todos los items asignados 
        $this->model->delete('STOCK_ITEMS_LOCATION', ' where  location="'.$idLoc.'" and ID_compania = "'.$this->model->id_compania.'"'); 
        $this->CheckError();
        ...
        echo 'La ubicación default del almacen principal no puede ser eliminado';
