  ...
  foreach ((array)$response->message as $key => $value) {
    ...
    if(!$this->checkSalesOrderExist($order_id)){
      ...
      $values = array(
        ...
        'TaxID'=> '',
        'OrderTax' => $value->header->tax == '' ? 0 : $value->header->tax,
...
public function oc_setItems() {
  ...
  foreach ($products as $key => $value) {
    ...
    $json[$i]['ean']   = $item->{$this->KeyStr('product.ean', $mapping)};
    $json[$i]['jan']   = $item->{$this->KeyStr('product.jan', $mapping)};
    $json[$i]['isbn']  = $item->{$this->KeyStr('product.isbn', $mapping)};
    $json[$i]['mpn']   = $item->{$this->KeyStr('product.mpn', $mapping)};
    $json[$i]['location']     = $item->{$this->KeyStr('product.location', $mapping)};
    $json[$i]['tax_class_id'] = $item->{$this->KeyStr('product.tax_class_id', $mapping)};
    $json[$i]['stock_status_id'] = $item->{$this->KeyStr('product.stock_status_id', $mapping)};
    $json[$i]['date_available'] = date('Y-m-d');
    $json[$i]['shipping'] = $item->{$this->KeyStr('product.shipping', $mapping)};
    $json[$i]['price']    = $item->{$this->KeyStr('product.price', $mapping)}; 
    $json[$i]['quantity'] = $item->{$this->KeyStr('product.quantity', $mapping)}; 
    $json[$i]['minimum'] = "1";
    $json[$i]['subtract'] = "1";
    $json[$i]['length'] = $item->{$this->KeyStr('product.length', $mapping)}; 
    $json[$i]['width'] = $item->{$this->KeyStr('product.width', $mapping)}; 
    $json[$i]['height'] =$item->{$this->KeyStr('product.height', $mapping)}; 
    $json[$i]['weight'] = $item->{$this->KeyStr('product.weight', $mapping)}; 
  ...
  $tblProducts = '<table id="mappingTable" class="table table-striped responsive table-bordered dataTable" cellspacing="0" role="grid" style="margin-left: 0px; width: 1699px;"><tr><th>ACIWEB</th><th>Opencart</th></tr>';
    ...
    foreach ($columns as $key => $value) {
      ...
      if(!in_array($value->Field, $noMappinCol)){
        ...
        $tblProducts .= "<option value='unassigned'  >Ninguno</option>";
