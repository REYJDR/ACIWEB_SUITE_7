...
public function Init(){
 ...
 $res = $this->model->verify_session();
        ...
        if($res=='0'){
            ...
            require APP . 'view/modules/billing/BillingCreditNotes.php';
...
$motivo = $this->model->Query_value('CREDITNOTE_HEADER','motivo','Where CreditNoteNumber="'.$filter->{'CreditNoteNumber'}.'" and  ID_compania="'.$this->model->id_compania.'" ');
...
public function SetCreditNoteHeader(){
...
list($CustomerID,$Subtotal,$TaxID,$Net_due,$nopo,$pago,$licitacion,$observaciones,$entrega,$ordertax,$fecha_entrega,$noinvoice,$printer,$motivo) = explode('@', $value );
...
public function ReadInvoiceFile($id_compania){
  ...
  $SQL = 'SELECT CreditNoteNumber,printer
            ...
            Where 
            ID_compania = "'.$id_compania.'" 
            AND ( GenCreditNumber IS NULL or GenCreditNumber = "" ) and date >= CURDATE() - 30';
...
private function getMotivosList(){
  
  $sql = 'SELECT * FROM CONF_NC_MOTIVO' ;
