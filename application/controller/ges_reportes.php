...
public function rep_so(){
  ...
  $res = $this->model->verify_session();
          ...
          if($res=='0'){
            ...
            require APP . 'view/mobile/rep_so.php';  
 ...
 $this->model->verify_session();
                ...
                if($date1!= 00-00-0000 ){
                 ...
                 $query ='SELECT * FROM `SalesOrder_Header_Imp` 
                    ...
                    inner JOIN `SAX_USER` ON `SAX_USER`.`id` = SalesOrder_Header_Imp.user '.$clause.' 
                    GROUP BY SalesOrder_Header_Imp.SalesOrderNumber 
                    order by SalesOrder_Header_Imp.LAST_CHANGE DESC limit 100' ; 
              ...
              echo '<ul class="responsive_table">
                    ...
                    <div class="table_section_14">Fecha</div>
                    <div class="table_section">Cliente</div> 
                    </li>';  
                    ...
                    foreach ($filter as $datos) {
                        ...
                        $filter = json_decode($datos);
                           ...
                           echo '<li class="table_row">
                                    <div class="table_section_14"><a  class="buy_now" href="'.URL.'index.php?url=ges_reportes/rep_so_detail/'.$filter->{'SalesOrderNumber'}.'" >'.$filter->{'SalesOrderNumber'}.'</a></div>
                                    <div class="table_section_14">'.$filter->{'date'}.'</div>
                                    <div class="table_section">'.$filter->{'CustomerID'}.'-'.$filter->{'CustomerName'}.'</div> 
                                 </li>';
...
public function rep_so_detail($so_number){
  ...
  WHERE SalesOrder_Header_Imp.SalesOrderNumber='".$so_number."' and SalesOrder_Header_Imp.ID_compania='".$this->model->id_compania."'  GROUP BY SalesOrder_Detail_Imp.SalesOrderNumber ";


  $ORDER_detail= $this->model->Query($query);

  $query_detail ="SELECT * FROM `SalesOrder_Detail_Imp`
  WHERE SalesOrder_Detail_Imp.SalesOrderNumber='".$so_number."' and  SalesOrder_Detail_Imp.ID_compania='".$this->model->id_compania."' ORDER BY SalesOrder_Detail_Imp.ItemOrd ASC ;";


  $ORDER= $this->model->Query($query_detail);
  

          if($res=='0'){
              // load views
            require APP . 'view/_templates/mob_header.php';  
            require APP . 'view/mobile/rep_so_detail.php';  
...
$clause.= 'WHERE PurOrdr_Header_Exp.ID_compania="'.$this->model->id_compania.'" AND  PurOrdr_Detail_Exp.ID_compania="'.$this->model->id_compania.'" AND PurOrdr_Header_Exp.PurchaseOrderNumber <> ""'; 
...
filter_reset_button_text: false}
  ...
  <table id="table_reportPurOrd"  class="display table  table-condensed table-striped table-bordered" >
   ...
   <tbody>';
    ...
    $oc = $this->model->get_OC_req_asc($sort,$limit,$clause);
